...
const DivideCategory:FC<DivideCategoryProps> = ({Usersettings,from,to}) => {
    const currency=Usersettings?.currency|| "INR"
    const divideQuery=useQuery<GetCategoryDivideType>({
        queryKey:['overview',"stats","categories",from,to],
        queryFn:async()=>{
           const response= await fetch(`api/divide/categoriesdivide?from=${DateCreate(from)}&to=${DateCreate(to)}`)
           if(!response.ok){
...
<>
    ...
    <DivCategory formmatter={formMatter} type="income" data={divideQuery.data||[]}/>
    <DivCategory formmatter={formMatter} type="expense" data={divideQuery.data||[]}/>
    </div>
    </>
  )
}
