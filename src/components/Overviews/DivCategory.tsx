        ...
        {filterData.map((item)=>{
            ...
            return (
                <div key={item.category} >
                   ...
                   <div>{item.categoryIcon} <span className='mx-2'>{item.category}</span>({percentage.toFixed(0)}%)
                   </div>
                   <div>
                   {formmatter.format(amount)}
                   </div>
                   </div>
                   <div className='my-2'><Progress value={percentage} indicator={type==="income" ? "bg-emerald-500":"bg-rose-400"}/></div>
                </div>
